...
- 動きを含む映像は、Driveレビューへ入れる前に実尺のmotion clip、8時点以上のcontact sheet、reviewContextを作る。静止画だけでアニメーション品質を承認しない。
...
- `reviewContext` には、各確認フレームで見えるべき役と人数、分離すべきシルエットの組、接触を許す接点、削除済みで再表示してはならない要素を列挙する。総合印象だけで合格にせず、この一覧を一項目ずつ確認して結果を残す。
