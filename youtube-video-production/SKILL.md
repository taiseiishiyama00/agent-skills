...
- 台本、分割レビューMP4、最終動画、サムネイル、投稿設定など、制作工程の成果物を人のレビューまたは承認へ渡す前に、成果物の種類や工程を問わず、作成者以外のレビューエージェントを最低1名置く。このルールは各工程へ重複記載せず、すべての人間レビューフェーズへ横断適用する。
- レビューエージェントには、対象成果物、正本となる入力、該当工程の実装契約と確認項目、既存のユーザー指摘を渡す。総合印象だけで判断させず、入力との一致、欠落、破綻、権利・安全域・同期などその工程で検証可能な品質を独立して確認させる。
- レビューエージェントが検出したblocker、major、入力契約違反、または明白な品質不足は、人へ見せる前に作成工程へ戻して修正する。修正後は成果物を再生成し、同じ観点で再レビューして、blocker、major、入力契約違反がゼロになってから人間レビューへ進める。
- エージェントレビューの判定、指摘、対応結果、対象成果物と正本入力のハッシュを、該当工程のレビュー成果物へ保存する。成果物または正本入力が変わった場合は判定を失効させ、最新の成果物で再実行する。
- この共通品質ゲートは人の承認を代行しない。レビューエージェントは `human` の承認を記録せず、品質を事前に引き上げたうえで判断材料と残存する留意点を人へ渡す。
