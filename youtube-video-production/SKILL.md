...
- 台本のエージェントレビューは自動実行しない。台本全体の完成後にユーザーからレビューを明示的に依頼された場合だけ行い、初稿提示、部分修正、コメント反映、再確認のたびには行わない。
- 台本レビューは現在の作業担当者が行ってよく、別のレビューエージェントを必須にしない。レビュー後の修正確認も、ユーザーから追加依頼がない限り自動で繰り返さない。
