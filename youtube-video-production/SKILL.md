...
- Google Gemini TTSが生成した音声は、renderer用コピーを通常1.25倍へ変換する。
- コント区間は1.5倍へ変換する。映像計画と字幕計画ではコント章の `id` を `skit` とし、速度判定を台本名や作品固有コードへ分散させない。
- 横動画とShortで速度を変えず、どちらにも通常1.25倍、コント1.5倍を適用する。
- Google Drive上の元WAVは変更しない。音声、発話、字幕cue、場面切替など音声時刻に依存する値を同じ比率で変換し、Remotion側で追加の `playbackRate` を重ねない。
- 速度はチャンネルprofileのTTS共通設定を正本とし、Short用設定や作品固有実装へ同じ数値を複製しない。
