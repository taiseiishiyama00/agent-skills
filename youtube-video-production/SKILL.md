...
AIはこれらを読み、MCP Toolを使うと明記されたフェーズでは対応Toolを一意の実行入口として使用する。内容ビジュアルは対応Toolで生成パラメータを取得した後、Chat/Codex内蔵の画像生成を使って作成する。画像生成と作品固有のRemotion実装にはMCP Toolを使わず、`workflow.md`の手順に従う。
