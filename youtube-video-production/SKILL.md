...
- 台本、音声、字幕、素材、全シーン画像、motion clip、確認用動画、最終動画、サムネイル、投稿設定など、制作工程の成果物を人のレビューまたは承認へ渡す前に、成果物の種類や工程を問わず、作成者以外のレビューエージェントを最低1名置く。このルールは各工程へ重複記載せず、すべての人間レビューフェーズへ横断適用する。
...
- アニメーション品質ゲートなど、作成者以外の複数エージェントとより厳しい合格条件を要求する工程は、その合格をこの共通品質ゲートの実施として扱う。同じ成果物へ追加の1名を重複配置する必要はない。
