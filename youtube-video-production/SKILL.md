...
- `reviewContext` には、各確認フレームで見えるべき役と人数、分離すべきシルエットの組、接触を許す接点、削除済みで再表示してはならない要素を列挙する。レビュアーは総合印象の採点だけでPASSにせず、この一覧を一項目ずつ確認する。
- ユーザーがマーカー、ラベル、人物などの削除を指示した場合は、該当する長尺・Shortsの両方へ不在テストを追加する。削除前の描画物が残るformatを一つでも確認した場合はレビューを開始しない。
- motion clip、contact sheet、実装ファイルのハッシュをレビュー開始直前に取り直し、実装変更後の最新ハッシュと一致しない証拠は失効させる。
