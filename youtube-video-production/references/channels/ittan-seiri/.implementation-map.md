...
> この文書はAIが管理する派生物である。[制作ワークフロー](workflow.md) と [出力要求](output-rules.md) を、実装・設定・テストへ対応付ける。要求や例外はこの文書で定義しない。

## 対応表

| 要求ID | 実現・検証手段 | 状態 |
|---|---|---|
| `workflow: Google Driveの構成`、`ITTAN-DATA-001` | pipelineのproject初期化、Drive同期、storage layout、固定出力先テスト | 要更新: `sources`、`outputs` 構成への対応 |
| `workflow: 横動画台本作成` | Google Docsの作成・同期 | 要更新: `sources/scripts` への対応 |
| `workflow: MP4作成・連結` | rendererの章別描画・連結、review task、Drive同期 | 要更新: 新しいレビュー配置への対応 |
| `workflow: 関連情報作成・投稿` | 投稿情報生成、YouTube操作、投稿結果検証 | 要更新: `outputs` への対応 |
| `ITTAN-LONG-001` | production contractで必須章ID・順序・時間値をplan検証し、Remotion taskへ要求IDを渡す | 一部未実装: 必須章・順序の自動検証 |
| `ITTAN-LONG-002`〜`005` | Google Docs台本、出典確認、台本メタ情報 | 一部自動化済み |
| `ITTAN-SHORT-001` | pipelineが横動画冒頭を完全コピーして定型CTAを付加し、コピー一致をテスト | 要更新: コピー範囲とCTA文言への対応 |
...
正本 `1.1.0` の実効発話速度は、句読点・記号・空白を除いた文字数を、utteranceの `endMs - startMs` 合計で割って検証する。
