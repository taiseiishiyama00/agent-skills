...
> この文書は派生手順書であり、台本要求の正本ではない。対象チャンネルの `channels/<channel-id>/video-standard.md` にある要求IDを、コメントと生成ミラーを壊さず再現する方法だけを定める。
...
話者名と文言は対象チャンネルの `video-standard.md` を正本とし、チャンネルprofileのShort台本テンプレートはその機械実装として一致させる。コピー区間をShort向けに要約、言い換え、短縮、強調し直さない。最大尺を超える場合も自動で書き換えず、横動画の冒頭を調整するか、正本の変更をユーザーへ確認する。
